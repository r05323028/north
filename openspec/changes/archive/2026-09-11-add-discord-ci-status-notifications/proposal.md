...
- **Invariant:** Read CI webhook URL from `DISCORD_CI_WEBHOOK` and a Discord GitHub-compatible pull-request webhook URL ending in `/github` from `DISCORD_PR_WEBHOOK`; never hard-code credentials or print them.
...
- Adds `.github/workflows/discord-ci-status.yml` and two optional repository secret configuration requirements.
