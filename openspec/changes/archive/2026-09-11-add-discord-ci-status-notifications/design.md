...
2. **Use runner-native `curl` and `jq`.** Construct the CI summary payload with `jq --arg` so branch and actor values remain data. For pull requests, forward `$GITHUB_EVENT_PATH` unchanged to Discord's GitHub-compatible `/github` endpoint so the required GitHub event envelope, including `sender`, remains intact. `curl` uses bounded retries and timeouts. This avoids an unpinned third-party action and adds no dependency.

3. **Use separate optional secrets.** Use `DISCORD_CI_WEBHOOK` for `workflow_run` notifications and a Discord GitHub-compatible webhook URL ending in `/github` in `DISCORD_PR_WEBHOOK` for pull request reminders. Select the corresponding secret from event type. Missing configuration emits an explicit skip message and succeeds; configured delivery errors remain visible on the notification workflow while never affecting the already-completed CI workflow.
...
- [Pull-request text reaches an external service] → The `/github` endpoint requires the original GitHub event envelope; forward only the supported `pull_request` event payload, with no checkout, source, tokens, or patch content.
...
2. Add repository Actions secrets `DISCORD_CI_WEBHOOK` for CI and `DISCORD_PR_WEBHOOK` for pull request reminders; configure the latter with the Discord webhook URL ending in `/github`.
