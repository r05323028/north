...
- Treat HTTP 409 as stale repair: refetch Requirement, fetch only an applicable
  packet for refreshed Ready state, preserve unsent Request Changes feedback, and
  require an explicit accessible refreshed-
  state acknowledgement before another mutation (`Review refreshed packet` for
  Ready decisions, or `Review refreshed Requirement` for Reopen). Bind that
  acknowledgement to `(requirement_state_version, assessment_id)` for Ready
  decisions or `requirement_state_version` for Reopen; unchanged refetches do
