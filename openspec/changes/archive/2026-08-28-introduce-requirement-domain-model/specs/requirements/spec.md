...
Request Changes; Rejected→Discussing as Reopen; plus agent-gated
Discussing→Ready). Illegal requests SHALL fail without side effects.
...
- **WHEN** a reviewer requests changes on a Ready requirement
- **THEN** status becomes Discussing with reviewer feedback preserved, and
the requirement is NOT Rejected
...
Persistence SHALL compare it atomically with the current row before applying a
domain operation. A stale value SHALL return a conflict (normally HTTP 409)
and SHALL persist no content, status, revision, audit, or other side effect.
