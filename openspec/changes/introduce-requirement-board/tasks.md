...
- [x] 1.2 Add the shared authenticated `GET /events` producer with post-commit `requirement.changed` hints; keep it lightweight, notification-only, non-durable, and independent of clarification. `Last-Event-ID` is not a correctness dependency; terminate a stream that overruns the broadcast buffer so EventSource reconnect/refetch can restore canonical state.
