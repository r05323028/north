...
- [ ] 2.5 Preserve persisted messages and refetch the detail bundle on start HTTP 409 or operational unavailability; use phase/status to distinguish awaiting-assignment retry, active-slot retention, and terminal new-start eligibility; if cancellation wins after message persistence, preserve the message and do not dispatch it; never retry with a newer state version or invent a local run.
