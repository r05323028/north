...
- [ ] 2.2 Use latest `/session` only to guide presentation; use `phase` to allow no-run `start`, awaiting-assignment same-message retry/cancel, active-run dispatch/cancel without competing start, or terminal-run new `start`; retain returned/public `run_id` and `start_message_id`, require known `run_id` for every later dispatch/cancel, and always send `expected_state_version` on start.
- [ ] 2.3 For a later persisted message, call `POST /requirements/{requirement_id}/clarification/runs/{run_id}/messages/{message_id}/dispatch` only for known `phase=active` `run_id`; prove run binding, repeated dispatch reuse, no second message, no status-only action inference, and no retargeting to a newer run.
- [ ] 2.4 Call `POST /requirements/{requirement_id}/clarification/runs/{run_id}/cancel` with known `run_id`; render separate `cancel_requested` intent, keep assigned `phase=active` through `command_ack`, render unassigned immediate `phase=terminal`, and never substitute latest-run identity.
- [ ] 2.5 Preserve persisted messages and refetch the detail bundle on start HTTP 409 or operational unavailability; use phase/status to distinguish awaiting-assignment retry, active-slot retention, and terminal new-start eligibility; never retry with a newer state version or invent a local run.
- [ ] 2.6 Add reconnect/refocus refetch for conversation and prove agent messages survive missed SSE hints.
- [ ] 2.7 Add UI coverage for reload retry using canonical `start_message_id`, phase-driven unavailable handling, assigned cancellation after `command_ack` without new run, terminal runtime release, sequential run creation, and explicit run identity preservation.
