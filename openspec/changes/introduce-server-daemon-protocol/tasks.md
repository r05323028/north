...
- [x] 1.2 Add `command.accepted`, event ACK accepted/rejected status, reconciliation watermark, and `protocol.error` frames; round-trip serialization tests cover every frame family.
...
- [ ] 2.3 Implement daemon event journal replay and server post-commit `event.accepted`/durable-rejection `event.rejected`; test duplicate assessment and ACK-after-commit behavior.
