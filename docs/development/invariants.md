...
| Public auth/setup request endpoints have resource-aware abuse limits | Partially Enforced | `north-server::public_abuse`, endpoint-isolated process buckets, trusted-proxy CIDR resolution, migration 0018 durable setup quota, generic 429, unit tests, and ignored PostgreSQL concurrency/HTTP tests; external database proof pending |
