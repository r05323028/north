...
| Public auth/setup request endpoints have resource-aware abuse limits | Enforced | `north-server::public_abuse`, endpoint-isolated process buckets, trusted-proxy CIDR resolution, migration 0018 durable setup quota, generic 429, unit tests, and executed public_endpoint_abuse PostgreSQL/HTTP tests and migration-upgrade proof |
